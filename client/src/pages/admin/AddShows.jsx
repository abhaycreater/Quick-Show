...
const AddShows = () => {
  ...
  try {
    ...
    if (data.success && Array.isArray(data.movies)) {
      ...
      setNowPlayingMovies([]); // fallback
    }

  } catch (error) {
    console.log('Error fetching movies', error);
    setNowPlayingMovies([]); // prevent crash
  }
};
    ...
    <>
      ...
      <div className='mt-6'>
        ...
        <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg'>
          ...
          <button onClick={handelDateTimeAdd} className='bg-primary/80 text-white px-3 py-2 text-sm rounded-lg hover:bg-primary cursor-pointer'>
          Add Time</button>
        </div>
      </div>
                ...
                <div className='flex flex-wrap gap-2 mt-1 text-sm'>
                  {times.map((time)=>(
                    <div key={time} className='border border-primary px-2 py-1 flex items-center rounded'>
                      <span>{time}</span>
                      <DeleteIcon onClick={()=>handelRemoveTime(date, time)} width={15} className='ml-2 text-red-500 hover:text-red-700 cursor-pointer'/>
                    </div>
                  ))}
                </div>
              </li>
      ...
      disabled={addingShow} className='bg-primary text-white px-8 py-2 mt-6 rounded hover:bg-primary/90 transition-all cursor-pointer'>
      Add Show</button>
    </>
  ): <Loading/>
}
